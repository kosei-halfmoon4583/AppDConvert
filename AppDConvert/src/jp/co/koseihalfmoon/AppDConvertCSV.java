 ...
 * SourceTree committed 2017/04/17
 *------------------------------------------------------------------------*/

public class AppDConvertCSV extends AppDScreenBase {
	...
	public static void main(String[] args){
		String name = "Cloudix売上/仕入れ情報 to AXシステム [ver 0.1.2]";
		int width = 680;
		int height = 300;
		@SuppressWarnings("unused")
		AppDConvertCSV ads = new AppDConvertCSV(name, width, height);
	}
	/** Constractor */
	AppDConvertCSV(String title, int x, int y) {
		super(title, x, y);
	...
	class BtnRegister implements ActionListener {
		public void actionPerformed(ActionEvent e)	{
			if(fName == "") {
				...
				//CSVUriageTemp csvrd = new CSVUriageTemp(fName, sName);
				csvrd.readers();
				/* 仕入れ一時ファイル作成 */
				CSVSiireTemp csvst = new CSVSiireTemp(fName);
				//CSVSiireTemp csvst = new CSVSiireTemp(fName, sName);
				csvst.readers();
			}		
		}
	}
			...
			if (selectedItem == mniSave) {
				...
				if (intRet == JFileChooser.APPROVE_OPTION) {
					try {
						...
						sName = myFileChooser.getSelectedFile().toString();
						txtEdit.append("\n");
						txtEdit.append(outputFile + sName);
